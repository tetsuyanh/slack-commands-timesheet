...
function doPost(e) {
  ...
  if (verificationToken != 'iN5URBeCG5R33Qcvt82ZUXbA') {
  ...
  var url = Utilities.formatString("%s#gid=%s", sheet.getParent().getUrl(), sheet.getSheetId());
  var resp = Utilities.formatString("saved you %s %s at %s.\n%s", verb, work, date, url);
...
function debugGetDate() {
  ...
  var now = new Date();
  Logger.log('date: %s', Utilities.formatDate(date, 'Asia/Tokyo', 'yyyy/MM/dd HH:mm:ss'));
  Logger.log('now : %s', Utilities.formatDate(now, 'Asia/Tokyo', 'yyyy/MM/dd HH:mm:ss'));
}

// 作業する月次シートを返す
function getMonthlySheet(work, date) {
  
  const sheetName = Utilities.formatString("%s_%s", work, Utilities.formatDate(date, 'Asia/Tokyo', 'yyyy/MM'));
  ...
  if (!sheet) {
    ...
    // 表示形式
    sheet.getRange(rowDateOffset, columnDate, rowDaysQuantity, 1).setNumberFormat('yyyy/MM/dd');
    sheet.getRange(rowDateOffset, columnStart, rowDaysQuantity, 1).setNumberFormat('H:mm:ss');
    sheet.getRange(rowDateOffset, columnFinish, rowDaysQuantity, 1).setNumberFormat('H:mm:ss');
    sheet.getRange(rowDateOffset, columnRest, rowDaysQuantity, 1).setNumberFormat('[h]:mm:ss');
    sheet.getRange(rowDateOffset, columnWorkTime, rowDaysQuantity, 1).setNumberFormat('[h]:mm:ss');
    
    // 計算式
    for (i = rowDateOffset; i < (rowDateOffset + rowDaysQuantity); i++) {
      var formulaWorkTime = Utilities.formatString("=IF(AND(NOT(ISBLANK(R[0]C[%s])), NOT(ISBLANK(R[0]C[%s]))), (R[0]C[%s]-R[0]C[%s]-R[0]C[%s]), \"\")",
                                                   finishR1C1,
                                                   startR1C1,
                                                   finishR1C1,
                                                   startR1C1,
    ...
    var formulaCnt = Utilities.formatString("=(ROWS(R[%s]C[0]:R[%s]C[0])-COUNTIF(R[%s]C[0]:R[%s]C[0],\"\"))",
                                            -rowDaysQuantity, 
                                            -1,
                                            -rowDaysQuantity, 
                                            -1
    ...
    var formulaAvr = Utilities.formatString("=SUM(R[%s]C[0]:R[%s]C[0])/(ROWS(R[%s]C[0]:R[%s]C[0])-COUNTIF(R[%s]C[0]:R[%s]C[0],\"\"))",
                                            -rowDaysQuantity-1,
                                            -2,
                                            -rowDaysQuantity-1,
                                            -2,
